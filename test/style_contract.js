...
const acknowledgedOverrides = new Set([...overrideManifest.matchAll(/^  ((?:_includes|_layouts)\/[^:]+):$/gm)].map((match) => match[1]));
const siteOwnedExtensions = new Set(["_includes/hook/bib.liquid", "_includes/localized-content.liquid"]);

for (const componentPath of ["_includes", "_layouts"]) {
  if (!exists(componentPath)) continue;

  const unexpectedFiles = listFiles(componentPath).filter((filePath) => !acknowledgedOverrides.has(filePath) && !siteOwnedExtensions.has(filePath));
  for (const filePath of unexpectedFiles) {
    failures.push(`Site component \`${filePath}\` must be site-owned or acknowledged in \`.al-folio-overrides.yml\`.`);
  }
}
